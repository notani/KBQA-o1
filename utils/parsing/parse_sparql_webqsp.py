        ...
        if lines[-1] == "LIMIT 1":
            ...
            for line in _tmp_body_lines:
                if compare_var in line:
                    if (
                        "FILTER" in line
                    ):  # the return var is also the argmax var, not covered by S-Expression
    ...
    def parse_naive_body(self, body_lines, filter_lines, ret_var, spec_condition=None):
        ...
        while len(successors):
            ...
            if len(dep_triplets) == 0:
                ...
                self.parse_assert(
                    len(dep_triplets) > 0
                )  # at least one dependancy triplets
                var_dep_list.append((tgt_var, dep_triplets))
...
def augment_with_s_expr_webqsp(split, check_execute_accuracy=False):
    ...
    dataset = load_json(f"dataset/WebQSP/origin/WebQSP.{split}.json")
    ...
    failed_instances = []
    accept_instances = []
    # use a dedicated tqdm instance so we can set postfix information
    ...
    for i, data in enumerate(pbar):
        ...
        for parse in parses:
            ...
            if flag_success:
                ...
                if check_execute_accuracy:
                    ...
                    try:
                        execute_ans = execute_query_with_odbc(
                            lisp_to_sparql(instance["SExpr"])
                        )
                        execute_ans = [
                            res.replace("http://rdf.freebase.com/ns/", "")
                            for res in execute_ans
                        ]
                        if "Answers" in parse:
                            gold_ans = [
                                ans["AnswerArgument"] for ans in parse["Answers"]
                            ]
                        else:
                        ...
                        if set(execute_ans) == set(gold_ans):
                            ...
                            data["SExpr_execute_right"] = True
                        instance["SExpr_execute_right"] = execute_right_flag
                    except Exception:
                        instance["SExpr_execute_right"] = execute_right_flag
            if split == "train":
                if flag_success and execute_right_flag:
                    accept_instances.append(instance)
                else:
                    failed_instances.append(instance)
            else:
                accept_instances.append(instance)
                if not (flag_success and execute_right_flag):
                    failed_instances.append(instance)
        ...
        if (i + 1) % update_freq == 0 or (i + 1) == len(dataset):
            ...
            if check_execute_accuracy and total_num:
                crate = (
                    f"{execute_hit_num}/{total_num} ({execute_hit_num / total_num:.4f})"
                )
