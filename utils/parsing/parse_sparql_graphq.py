...
class Parser:
    ...
    def normalize_body_lines(self, lines, filter_string_flag=False):
        ...
        if re.match(r"FILTER \(\?\w* (>|<|>=|<=) .*", lines[-2]) or re.match(
            r"FILTER \(xsd:integer\(\?\w*\) (>|<|>=|<=) .*", lines[-2]
        ):
            ...
            compare_condition = [
                "COMPARATIVE",
                operator_mapper[compare_operator],
                compare_var,
                compare_value,
            ]
        ...
        if start_line != -1:
            ...
            if end_line == start_line + 12:  # discard redundant range filters
                lines = (
                    lines[:start_line]
                    + lines[end_line:right_parantheses_line]
                    + lines[start_line : end_line - 6]
                    + lines[right_parantheses_line:]
                )
            else:
                lines = (
                    lines[:start_line]
        ...
        if lines[-1] == "LIMIT 1":
            ...
            hit = False
            for line in _tmp_body_lines:
                if compare_var in line:
                    # self.parse_assert(l.endswith('?sk0 .') and not hit)
                    self.parse_assert(
                        line.endswith(compare_var + " .") and not hit
                    )  # appear only once
                    hit = True
                    arg_var, arg_r = line.split(" ")[0], line.split(" ")[1]
                    arg_r = arg_r[3:]  # rm ns:
                else:
        ...
        if body_lines:  # already processed by superlative extraction
            ...
            range_line_num = -7
        if len(lines) >= abs(range_line_num) and lines[range_line_num].startswith(
            "FILTER(NOT EXISTS {?"
        ):
            # WHERE {
            # ns:m.04f_xd8 ns:government.government_office_or_title.office_holders ?y .
            ...
            range_prompt = range_prompt[
                range_prompt.index("{") + 1 : range_prompt.index("}")
            ]
    ...
    def dep_graph_to_s_expr(self, var_dep_list, ret_var, spec_condition=None):
        ...
        spec_var_map = (
            {cond[2]: i for i, cond in enumerate(spec_condition)}
            if spec_condition
            else None
        )
        ...
        for var_name, dep_relations in var_dep_list:
            ...
            if spec_var_map and var_name in spec_var_map:  # spec_condition
                ...
                if cond[0] == "SUPERLATIVE":
                    ...
                    relation = cond[3]
                    clause = "{} ({}) {}".format(cond[1].upper(), clause, relation)
                elif cond[0] == "RANGE":
    ...
    def parse_naive_body(self, body_lines, filter_lines, ret_var, spec_condition=None):
        ...
        triplets = [
            x[:2] + [" ".join(x[2:-1]), x[-1]] if len(x) > 4 else x for x in triplets
        ]  # avoid error splitting like "2100 Woodward Avenue"@en
        triplets = [x[:-1] if x[-1] == "." else x for x in triplets]  # remove '.'

        # remove ns
        triplets = [
            [x[3:] if x.startswith("ns:") else x for x in tri] for tri in triplets
        ]
        # dependancy graph
        ...
        while len(successors):
            ...
            if len(dep_triplets) == 0:
                ...
                self.parse_assert(len(dep_triplets) > 0)  # at least dependancy triplets
                var_dep_list.append((tgt_var, dep_triplets))
                        ...
                        if filter_lines:  # check filter variable `?sk0`
                            ...
                            for line in filter_lines:
                                if successor_var in line:
                                    ...
                                    line = line.replace("FILTER (str(", "").replace(
                                        ")", ""
                                    )
...
GraphQA_FIX_DICT = {
    "(ARGMIN astronomy.spectral_type (R astronomy.star.spectral_type) astronomy.celestial_object.right_ascention)": "(ARGMIN astronomy.spectral_type (JOIN (R astronomy.star.spectral_type) astronomy.celestial_object.right_ascention))",
    "(ARGMIN astronomy.asteroid_group (R astronomy.asteroid.member_of_asteroid_group) astronomy.celestial_object.absolute_magnitude)": "(ARGMIN astronomy.asteroid_group (JOIN (R astronomy.asteroid.member_of_asteroid_group) astronomy.celestial_object.absolute_magnitude))",
    "(ARGMIN digicams.digital_camera_manufacturer digicams.digital_camera_manufacturer.cameras digicams.digital_camera.lcd_pixels)": "(ARGMIN digicams.digital_camera_manufacturer (JOIN digicams.digital_camera_manufacturer.cameras digicams.digital_camera.lcd_pixels))",
    "(ARGMAX spaceflight.rocket_engine_designer (R spaceflight.rocket_engine.designed_by) spaceflight.rocket_engine.thrust_sea_level)": "(ARGMAX spaceflight.rocket_engine_designer (JOIN (R spaceflight.rocket_engine.designed_by) spaceflight.rocket_engine.thrust_sea_level))",
    "(ARGMAX (AND baseball.baseball_player (JOIN baseball.baseball_player.lifetime_batting_statistics (JOIN baseball.lifetime_batting_statistics.hits 1240^^http://www.w3.org/2001/XMLSchema#integer))) baseball.baseball_player.lifetime_batting_statistics baseball.lifetime_batting_statistics.grand_slams)": "(ARGMAX (AND baseball.baseball_player (JOIN baseball.baseball_player.lifetime_batting_statistics (JOIN baseball.lifetime_batting_statistics.hits 1240^^http://www.w3.org/2001/XMLSchema#integer))) (JOIN baseball.baseball_player.lifetime_batting_statistics baseball.lifetime_batting_statistics.grand_slams))",
}


def augment_with_s_expr_graphq(split, check_execute_accuracy=False):
    """augment original graphq datasets with s-expression"""
    if split == "train" or split == "test":
        if split == "train":
            dataset = load_json(
                "dataset/GraphQ/origin/graphquestions_v1_fb15_training_091420.json"
            )
        elif split == "test":
            dataset = load_json(
                "dataset/GraphQ/origin/graphquestions_v1_fb15_test_091420.json"
            )
        total_num = 0
        hit_num = 0
        execute_hit_num = 0
        pbar = tqdm(dataset, total=len(dataset))
        update_freq = 10  # how often (in items) to refresh postfix
        for i, data in enumerate(pbar):
            s_expression = data["s_expression"]
            if s_expression in GraphQA_FIX_DICT:
                data["s_expression"] = GraphQA_FIX_DICT[s_expression]
            data["SExpr_execute_right"] = False
            total_num += 1
            hit_num += 1
            if check_execute_accuracy:
                ...
                try:
                    ...
                    data["SExpr_execute_right"] = execute_right_flag
                except Exception:
                    data["SExpr_execute_right"] = execute_right_flag
                data["answer"] = gold_ans
            dataset[i]["sparql_query"] = lisp_to_sparql(data["s_expression"])

            # update progress bar postfix periodically to avoid excessive updates
            if (i + 1) % update_freq == 0 or (i + 1) == len(dataset):
                if total_num:
                    srate = f"{hit_num}/{total_num} ({hit_num / total_num:.4f})"
                else:
                    srate = "0/0 (0.0000)"
                if check_execute_accuracy and total_num:
                    crate = f"{execute_hit_num}/{total_num} ({execute_hit_num / total_num:.4f})"
                    # use set_postfix to show both rates on the right side of the bar
